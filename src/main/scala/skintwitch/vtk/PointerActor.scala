...
extends AnimatedActor {
  ...
  private def update() {
    ...
    val (dist, contactPt) = triMesh.signedDistanceTo(mTip.co(sample))
    ...
    if (dist < 0) distActor.GetProperty.SetColor(1.0, 0.5, 0.5)
    else distActor.GetProperty.SetColor(1.0, 1.0, 1.0)
    distPoints.Modified
  }
